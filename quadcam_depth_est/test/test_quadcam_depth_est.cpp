...
void drawGrid(cv::Mat& show, int num_rows = 50) {
  for (int i = 0; i < num_rows; i++) {
    ...
    cv::line(show, cv::Point(i * show.cols / num_rows, 0),
             cv::Point(i * show.cols / num_rows, show.rows - 1), c, 1);
  }
}
...
int main(int argc, char** argv) {
  ...
  desc.add_options()("help", "produce help message")(
      "image_l,l", po::value<std::string>()->default_value(""),
      "path of test image l")("image_r,r",
                              po::value<std::string>()->default_value(""),
                              "path of test image r")(
      "calib,c", po::value<std::string>()->default_value(""),
      "path of camera config")("name0",
                               po::value<std::string>()->default_value("cam3"),
                               "name of camera_left")(
      "name1", po::value<std::string>()->default_value("cam2"),
      "name of camera_right")("idx0", po::value<int>()->default_value(1),
                              "idx of camera_left")(
      ...
      "engine,e", po::value<std::string>()->default_value(""),
      "engine of onnx");
  po::variables_map vm;
  ...
  cv::imshow("Raw", show);
  cv::waitKey(1);
  printf("Read image file %s %s OK", image_path_l.c_str(),
         image_path_r.c_str());
  YAML::Node config = YAML::LoadFile(calib_path);
  auto ret_left =
      D2FrontEnd::readCameraConfig(camera_name0, config[camera_name0]);
  auto ret_right =
      D2FrontEnd::readCameraConfig(camera_name1, config[camera_name1]);

  // Initialize undistort
  FisheyeUndist undistort2_0(ret_left.first, 0, fov, true,
                             FisheyeUndist::UndistortPinhole2, width, height);
  FisheyeUndist undistort2_1(ret_right.first, 0, fov, true,
                             FisheyeUndist::UndistortPinhole2, width, height);
  ...
  VirtualStereo virtual_stereo(0, 1, Swarm::Pose(), &undistort2_0,
                               &undistort2_1, idx0, idx1, nullptr, nullptr);
  ...
  int num_rows = 10;
  for (int i = 0; i < num_rows; i++) {
    cv::line(show, cv::Point(0, i * show.rows / num_rows),
             cv::Point(show.cols - 1, i * show.rows / num_rows),
             cv::Scalar(0, 255, 0), 1);
  }
  ...
  // Draw vertical lines
  drawGrid(show);
  cv::namedWindow("Rectified Images2",
                  cv::WINDOW_NORMAL | cv::WINDOW_GUI_EXPANDED);
