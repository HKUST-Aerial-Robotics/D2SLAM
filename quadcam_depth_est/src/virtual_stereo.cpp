...
VirtualStereo::VirtualStereo(int _cam_idx_a, int _cam_idx_b,
    ...
    : cam_idx_a(_cam_idx_a),
      cam_idx_b(_cam_idx_b),
      undist_left(_undist_left),
      undist_right(_undist_right),
      undist_id_l(_undist_id_l),
      undist_id_r(_undist_id_r),
      hitnet(_hitnet),
      crestereo(_crestereo) {
  auto cam_param =
      static_cast<const camodocal::PinholeCamera*>(undist_left->cam_side.get())
          ->getParameters();
  img_size = cv::Size(cam_param.imageWidth(), cam_param.imageHeight());
  cv::Mat K = (cv::Mat_<double>(3, 3) << cam_param.fx(), 0, cam_param.cx(), 0,
               cam_param.fy(), cam_param.cy(), 0, 0, 1);
  initRecitfy(baseline, K, cv::Mat(), K, cv::Mat());
}

void VirtualStereo::initVingette(const cv::Mat& _inv_vingette_l,
                                 const cv::Mat& _inv_vingette_r) {
...
VirtualStereo::VirtualStereo(const Swarm::Pose& baseline,
  ...
  if (cam_left->modelType() == camodocal::Camera::ModelType::MEI) {
    ...
    auto cam_param = static_cast<const camodocal::CataCamera*>(cam_left.get())
                         ->getParameters();
    img_size = cv::Size(320, 240);
    double s = 0.0;
    K0 = (cv::Mat_<double>(3, 3) << cam_param.gamma1(), s, cam_param.u0(), 0,
          cam_param.gamma2(), cam_param.v0(), 0, 0, 1);
    D0 = (cv::Mat_<double>(1, 4) << cam_param.k1(), cam_param.k2(),
          cam_param.p1(), cam_param.p2());
    if (flag == cv::omnidir::RECTIFY_LONGLATI) {
      P1 = (cv::Mat_<double>(3, 3) << img_size.width / 3.1415, 0, 0, 0,
            img_size.height / 3.1415, 0, 0, 0, 1);
    } else {
      P1 = (cv::Mat_<double>(3, 3) << img_size.width / 4, 0, img_size.width / 2,
            0, img_size.height / 4, img_size.height / 2, 0, 0, 1);
    }
    P2 = P1;
    xi0 = cam_param.xi();
    cam_param = static_cast<const camodocal::CataCamera*>(cam_right.get())
                    ->getParameters();
    K1 = (cv::Mat_<double>(3, 3) << cam_param.gamma1(), s, cam_param.u0(), 0,
          cam_param.gamma2(), cam_param.v0(), 0, 0, 1);
    D1 = (cv::Mat_<double>(1, 4) << cam_param.k1(), cam_param.k2(),
          cam_param.p1(), cam_param.p2());
    xi1 = cam_param.xi();
    cv::stereoRectify(P1, cv::Mat(), P1, cv::Mat(), img_size, R, T, R1, R2, P1,
                      P2, Q, 1024, -1, cv::Size());
  } else {
    printf("Unsupported camera model:", cam_left->modelType());
  }
  cv::omnidir::initUndistortRectifyMap(K0, D0, xi0, R1, P1, img_size, CV_32FC1,
                                       lmap_1, lmap_2, flag);
  cv::omnidir::initUndistortRectifyMap(K1, D1, xi1, R2, P2, img_size, CV_32FC1,
                                       rmap_1, rmap_2, flag);
...
void VirtualStereo::initRecitfy(const Swarm::Pose& baseline, cv::Mat K0,
                                cv::Mat D0, cv::Mat K1, cv::Mat D1) {
  cv::eigen2cv(baseline.R(), R);
  cv::eigen2cv(baseline.pos(), T);
  cv::stereoRectify(K0, D0, K1, D1, img_size, R, T, R1, R2, T1, T2, Q, 1024, -1,
                    cv::Size(), &roi_l, &roi_r);
...
std::pair<cv::Mat, cv::Mat> VirtualStereo::estimatePointsViaRaw(
    const cv::Mat& left, const cv::Mat& right, const cv::Mat& left_color,
    bool show) {
...
std::pair<cv::Mat, cv::Mat> VirtualStereo::estimateDisparityViaRaw(
    const cv::Mat& left, const cv::Mat& right, const cv::Mat& left_color,
    bool show) {
  ...
  if (enable_texture) {
    // cv::cuda::remap(lcolor_gpu, lcolor_gpu, cuda_lmap_1, cuda_lmap_2,
    // cv::INTER_LINEAR);
    if (input_is_stereo) {
      cv::Mat lcolor;
      if (limg_rect.channels() == 1) {
...
std::vector<cv::cuda::GpuMat> VirtualStereo::rectifyImage(
  ...
  cv::cuda::remap(img_cuda_l, leftRectify, cuda_lmap_1, cuda_lmap_2,
                  cv::INTER_LINEAR);
  cv::cuda::remap(img_cuda_r, rightRectify, cuda_rmap_1, cuda_rmap_2,
                  cv::INTER_LINEAR);
  return {leftRectify, rightRectify};
}

cv::Mat VirtualStereo::estimateDisparityOCV(const cv::Mat& left,
                                            const cv::Mat& right) {
  auto sgbm = cv::StereoSGBM::create(
      config.minDisparity, config.numDisparities, config.blockSize, config.P1,
      config.P2, config.disp12MaxDiff, config.preFilterCap,
      config.uniquenessRatio, config.speckleWindowSize, config.speckleRange,
      config.mode);
  cv::Mat disparity;
...
cv::Mat VirtualStereo::estimateDisparity(const cv::Mat& left,
                                         const cv::Mat& right) {
