...
QuadCamDepthEst::QuadCamDepthEst(ros::NodeHandle& _nh) : nh(_nh) {
  ...
  nh.param<std::string>("quadcam_depth_config_file", quadcam_depth_config_file,
                        "quadcam_depth_config.yaml");
  nh.param<bool>("show", show, false);
  pub_pcl = nh.advertise<sensor_msgs::PointCloud2>(
      "/depth_estimation/point_cloud", 1);
  ...
  if (camera_config == CameraConfig::FOURCORNER_FISHEYE) {
    image_sub = it_->subscribe("/arducam/image", 1000,
                               &QuadCamDepthEst::imageCallback, this, hints);
  } else {
    left_sub = new ImageSubscriber(*it_, "/cam0/image_raw", 1000, hints);
    right_sub = new ImageSubscriber(*it_, "/cam1/image_raw", 1000, hints);
    sync = new message_filters::TimeSynchronizer<sensor_msgs::Image,
                                                 sensor_msgs::Image>(
        *left_sub, *right_sub, 1000);
    sync->registerCallback(
        boost::bind(&QuadCamDepthEst::stereoImagesCallback, this, _1, _2));
  }
  if (enable_texture) {
...
void QuadCamDepthEst::loadCNN(YAML::Node& config) {
  ...
  if (enable_cnn) {
    ...
    if (cnn_type == "hitnet") {
      hitnet = new HitnetONNX(cnn_model_path, width, height, cnn_use_tensorrt,
                              cnn_fp16, cnn_int8);
      cnn_rgb = false;
    }
    if (cnn_type == "crestereo") {
      crestereo = new CREStereoONNX(cnn_model_path, width, height,
                                    cnn_use_tensorrt, cnn_fp16, cnn_int8);
      cnn_rgb = true;
    }
    printf(
        "[QuadCamDepthEst] Load CNN from %s tensorrt %d FP16 %d INT8 %d width "
        "%d height %d\n",
        cnn_model_path.c_str(), cnn_use_tensorrt, cnn_fp16, cnn_int8, width,
        height);
...
void QuadCamDepthEst::stereoImagesCallback(
  ...
  cv_bridge::CvImagePtr cv_ptr_l =
      cv_bridge::toCvCopy(left, sensor_msgs::image_encodings::MONO8);
  cv_bridge::CvImagePtr cv_ptr_r =
      cv_bridge::toCvCopy(right, sensor_msgs::image_encodings::MONO8);
  if (enable_texture) {
  ...
  std::pair<cv::Mat, cv::Mat> ret = virtual_stereos[0]->estimatePointsViaRaw(
      cv_ptr_l->image, cv_ptr_r->image, cv_ptr_l->image, show);
  if (enable_texture) {
    addPointsToPCL(ret.first, ret.second, virtual_stereos[0]->extrinsic,
                   *pcl_color, pixel_step, min_z, max_z);
  } else {
    addPointsToPCL(ret.first, ret.second, virtual_stereos[0]->extrinsic, *pcl,
                   pixel_step, min_z, max_z);
  }
  if (show) {
    cv::waitKey(1);
  }
...
void QuadCamDepthEst::imageCallback(const sensor_msgs::ImageConstPtr& left) {
  ...
  for (auto stereo : virtual_stereos) {
    ...
    if (cnn_rgb) {
      ret = stereo->estimatePointsViaRaw(
          imgs[stereo->cam_idx_a], imgs[stereo->cam_idx_b], cv::Mat(), show);
    } else {
      ret = stereo->estimatePointsViaRaw(imgs_gray[stereo->cam_idx_a],
                                         imgs_gray[stereo->cam_idx_b],
                                         imgs[stereo->cam_idx_a], show);
    }
    if (enable_texture) {
      addPointsToPCL(ret.first, ret.second, stereo->extrinsic, *pcl_color,
                     pixel_step, min_z, max_z);
    } else {
      addPointsToPCL(ret.first, ret.second, stereo->extrinsic, *pcl, pixel_step,
                     min_z, max_z);
    }
  }
  if (show) {
...
void QuadCamDepthEst::loadCameraConfig(YAML::Node& config,
  ...
  if (config["photometric_calib"]) {
    photometric_inv = readVingette(
        configPath + "/" + config["photometric_calib"].as<std::string>(),
        avg_brightness);
  }
  if (config["photometric_calib_1"]) {
    photometric_inv_1 = readVingette(
        configPath + "/" + config["photometric_calib_1"].as<std::string>(),
        avg_brightness);
  }
  ...
  for (const auto& kv : config_cams) {
    ...
    if (camera_config == CameraConfig::FOURCORNER_FISHEYE) {
      ...
      undistortors.push_back(new D2Common::FisheyeUndist(
          ret.first, 0, fov, true, D2Common::FisheyeUndist::UndistortPinhole2,
          width, height, photometric_inv));
    }
  ...
  if (camera_config == CameraConfig::STEREO_PINHOLE) {
    ...
    auto stereo = new VirtualStereo(baseline, raw_cameras[0], raw_cameras[1],
                                    hitnet, crestereo);
    ...
    for (const auto& kv : config["stereos"]) {
      ...
      std::string stereo_calib_file =
          configPath + "/" + node["stereo_config"].as<std::string>();
      Swarm::Pose baseline;
      ...
      printf(
          "[QuadCamDepthEst] Load stereo %s, stereo %d(%d):%d(%d) baseline: "
          "%s\n",
          stereo_name.c_str(), cam_idx_l, idx_l, cam_idx_r, idx_r,
          baseline.toStr().c_str());
      auto stereo = new VirtualStereo(
          cam_idx_l, cam_idx_r, baseline, undistortors[cam_idx_l],
          undistortors[cam_idx_r], idx_l, idx_r, hitnet, crestereo);
      auto att = undistortors[cam_idx_l]->t[idx_l];
      stereo->extrinsic =
          raw_cam_extrinsics[cam_idx_l] * Swarm::Pose(att, Vector3d(0, 0, 0));
      stereo->enable_texture = enable_texture;
      stereo->initRecitfy(baseline, KD0.first, KD0.second, KD1.first,
                          KD1.second);
      virtual_stereos.emplace_back(stereo);
    }
  }
