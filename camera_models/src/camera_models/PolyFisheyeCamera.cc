...
PolyFisheyeCamera::PolyFisheyeCamera(const std::string& cameraName,
    ...
    : mParameters(cameraName, imageWidth, imageHeight, k2, k3, k4, k5, k6, k7,
                  p1, p2, A11, A12, A22, u0, v0, isFast) {
  ...
  if (mParameters.isFast() == 1) {
    ...
    fastCalc = new FastCalcTABLE(coeff, mParameters.numDiff(),
...
Camera::ModelType PolyFisheyeCamera::modelType() const {
  return mParameters.modelType();
}

const std::string& PolyFisheyeCamera::cameraName() const {
  return mParameters.cameraName();
}

int PolyFisheyeCamera::imageWidth() const { return mParameters.imageWidth(); }

int PolyFisheyeCamera::imageHeight() const { return mParameters.imageHeight(); }
...
void PolyFisheyeCamera::estimateIntrinsics(
...
void PolyFisheyeCamera::liftSphere(const Eigen::Vector2d& p,
...
void PolyFisheyeCamera::liftProjectiveToRay(const Eigen::Vector2d& p,
                                            Ray& ray) const {
  // TODO
...
void PolyFisheyeCamera::liftProjective(const Eigen::Vector2d& p,
                                       Eigen::Vector3d& P) const {
  ...
  if (mParameters.isDistortion()) {
    if (mParameters.isFast() == 1) {
      fastCalc->backprojectSymmetric(
          Eigen::Vector2d(m_inv_K11 * p(0) + m_inv_K12 * p(1) + m_inv_K13,
                          m_inv_K22 * p(1) + m_inv_K23),
          cos_theta, sin_theta, cos_phi, sin_phi);
      //            std::cout << " cos_theta fastCalc " << cos_theta <<
      //            std::endl;
    } else {
      backprojectSymmetric(
          Eigen::Vector2d(m_inv_K11 * p(0) + m_inv_K12 * p(1) + m_inv_K13,
    ...
    Eigen::Vector2d p_u(m_inv_K11 * p(0) + m_inv_K13,
                        m_inv_K22 * p(1) + m_inv_K23);
...
void PolyFisheyeCamera::undistToPlane(const Eigen::Vector2d& p_u,
                                      Eigen::Vector2d& p) const {}

cv::Mat PolyFisheyeCamera::initUndistortRectifyMap(cv::Mat& map1, cv::Mat& map2,
                                                   float fx, float fy,
...
int PolyFisheyeCamera::parameterCount() const { return FISHEYE_PARAMS_NUM; }

const PolyFisheyeCamera::Parameters& PolyFisheyeCamera::getParameters() const {
  return mParameters;
}

void PolyFisheyeCamera::setParameters(
...
void PolyFisheyeCamera::readParameters(
...
void PolyFisheyeCamera::writeParameters(
...
void PolyFisheyeCamera::writeParametersToYamlFile(
    const std::string& filename) const {
  mParameters.writeToYamlFile(filename);
}

std::string PolyFisheyeCamera::parametersToString() const {
...
bool PolyFisheyeCamera::calcKinvese(double a11, double a12, double a22,
                                    double u0, double v0) {
...
void PolyFisheyeCamera::setPoly(math_utils::Polynomial* value) { poly = value; }

PolyFisheyeCamera::FastCalcTABLE* PolyFisheyeCamera::getFastCalc() {
  return fastCalc;
}
...
double PolyFisheyeCamera::getInv_K23() const { return m_inv_K23; }

double PolyFisheyeCamera::getInv_K22() const { return m_inv_K22; }

double PolyFisheyeCamera::getInv_K13() const { return m_inv_K13; }

double PolyFisheyeCamera::getInv_K12() const { return m_inv_K12; }

double PolyFisheyeCamera::getInv_K11() const { return m_inv_K11; }

math_utils::Polynomial* PolyFisheyeCamera::getPoly() const { return poly; }
...
double& PolyFisheyeCamera::Parameters::k2() { return m_k2; }

double& PolyFisheyeCamera::Parameters::k3() { return m_k3; }

double& PolyFisheyeCamera::Parameters::k4() { return m_k4; }

double& PolyFisheyeCamera::Parameters::k5() { return m_k5; }

double& PolyFisheyeCamera::Parameters::k6() { return m_k6; }

double& PolyFisheyeCamera::Parameters::k7() { return m_k7; }

double& PolyFisheyeCamera::Parameters::p1() { return m_p1; }

double& PolyFisheyeCamera::Parameters::p2() { return m_p2; }

double& PolyFisheyeCamera::Parameters::A11() { return m_A11; }

double& PolyFisheyeCamera::Parameters::A12() { return m_A12; }

double& PolyFisheyeCamera::Parameters::A22() { return m_A22; }

double& PolyFisheyeCamera::Parameters::u0() { return m_u0; }

double& PolyFisheyeCamera::Parameters::v0() { return m_v0; }

int& PolyFisheyeCamera::Parameters::numDiff() { return m_numDiff; }

int& PolyFisheyeCamera::Parameters::maxIncidentAngle() {
  return m_maxIncidentAngle;
}

int& PolyFisheyeCamera::Parameters::isFast() { return m_isFast; }

double PolyFisheyeCamera::Parameters::k2() const { return m_k2; }

double PolyFisheyeCamera::Parameters::k3() const { return m_k3; }

double PolyFisheyeCamera::Parameters::k4() const { return m_k4; }

double PolyFisheyeCamera::Parameters::k5() const { return m_k5; }

double PolyFisheyeCamera::Parameters::k6() const { return m_k6; }

double PolyFisheyeCamera::Parameters::k7() const { return m_k7; }

double PolyFisheyeCamera::Parameters::p1() const { return m_p1; }

double PolyFisheyeCamera::Parameters::p2() const { return m_p2; }

double PolyFisheyeCamera::Parameters::A11() const { return m_A11; }

double PolyFisheyeCamera::Parameters::A12() const { return m_A12; }

double PolyFisheyeCamera::Parameters::A22() const { return m_A22; }

double PolyFisheyeCamera::Parameters::u0() const { return m_u0; }

double PolyFisheyeCamera::Parameters::v0() const { return m_v0; }

int PolyFisheyeCamera::Parameters::isFast() const { return m_isFast; }

int PolyFisheyeCamera::Parameters::numDiff() const { return m_numDiff; }

int PolyFisheyeCamera::Parameters::maxIncidentAngle() const {
  return m_maxIncidentAngle;
}

void PolyFisheyeCamera::Parameters::setFast() { m_isFast = 1; }

bool PolyFisheyeCamera::Parameters::readFromYamlFile(
...
void PolyFisheyeCamera::Parameters::writeToYamlFile(
    const std::string& filename) const {
...
PolyFisheyeCamera::Parameters& PolyFisheyeCamera::Parameters::operator=(
...
eigen_utils::Matrix PolyFisheyeCamera::FastCalcTABLE::getMatAngleToR() {
  return angleToR;
}

eigen_utils::Matrix PolyFisheyeCamera::FastCalcTABLE::getMatRToAngle() {
  return rToAngle;
}

double PolyFisheyeCamera::FastCalcTABLE::getMaxIncidentAngle() {
  return maxIncidentAngle;
}

int PolyFisheyeCamera::FastCalcTABLE::getNumDiff() { return numDiffR; }

double PolyFisheyeCamera::FastCalcTABLE::getDiffAngle() { return diffAngle; }

double PolyFisheyeCamera::FastCalcTABLE::getDiffR() { return diffR; }

bool PolyFisheyeCamera::FastCalcTABLE::calcAngleToR(
    eigen_utils::Matrix& _angleToR, const int _numDiffAngle,
...
#define BACK "\033[0m"
      std::cout << RED << "#ERROR: polynomial NOT monotone in field!" << BACK
                << std::endl;
      std::cout << RED << "#ERROR: Maybe Wrong 'maxIncidentAngle' input!"
                << BACK << std::endl;
      std::cout << RED << "#ERROR: Projection polynomial ERROR!" << BACK
                << std::endl;
...
bool PolyFisheyeCamera::FastCalcTABLE::calcRToAngle(
    eigen_utils::Matrix& _rToAngle, const int _numDiffR, const double _diffR,
...
void PolyFisheyeCamera::FastCalcTABLE::backprojectSymmetric(
    const Eigen::Vector2d& p_u, double& cos_theta, double& sin_theta,
    double& cos_phi, double& sin_phi) const {
  double r = p_u.norm();
  double theta;
  ...
  if (r < 1e-10) {
    ...
    //  std::cout << " r " << r << " diffR " << diffR << " num " << num << "
    //  numDiffR "
    //  << numDiffR
    //  << std::endl;
    ...
    if (num >= numDiffR || num_up >= numDiffR) {
      ...
      theta = theta_down + (num - (double)num_down) * (theta_up - theta_down) /
                               (num_up - num_down);
...
double PolyFisheyeCamera::FastCalcTABLE::r(const double theta) const {
  if (theta > 1e-10 && theta < maxIncidentAngle) {
    ...
    if (num >= numDiffAngle || num_up >= numDiffAngle) {
      ...
      // linearlize the line with more than 1000 segment
      return (r_down +
              (num - (double)num_down) * (r_up - r_down) / (num_up - num_down));
      //        std::cout << "theta "<< theta_down<< " " <<theta_up << " "
      //        <<theta
      //        <<std::endl;
    }
...
void PolyFisheyeCamera::FastCalcPOLY::backprojectSymmetric(
    const Eigen::Vector2d& p_u, double& cos_theta, double& sin_theta,
    double& cos_phi, double& sin_phi) const {
  double r = p_u.norm();
  //  std::cout << "#INFO: r is " << r << std::endl;
...
double PolyFisheyeCamera::FastCalcPOLY::r(const double theta) const {
  if (theta > 1e-10 && theta < maxIncidentAngle) {
