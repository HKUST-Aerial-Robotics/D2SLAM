...
Ray::Ray(double x, double y, double z)
    : m_theta(acos(z / sqrt(x * x + y * y + z * z))), m_phi(atan2(y, x)) {}

Ray::Ray(Eigen::Vector3d P)
    : m_theta(acos(P(2) / P.norm())), m_phi(atan2(P(1), P(0))) {}
...
Eigen::Vector3d Ray::toSpace() const {
  return Eigen::Vector3d(sin(m_theta) * cos(m_phi), sin(m_theta) * sin(m_phi),
                         cos(m_theta));
}
