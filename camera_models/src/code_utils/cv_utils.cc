...
cv_utils::fisheye::PreProcess::PreProcess(const cv::Size _raw_image_size,
    ...
    : is_preprocess(false), is_resize_only(false) {
  /* clang-format off */
...
void cv_utils::fisheye::PreProcess::resetPreProcess(cv::Size _roi_size,
                                                    cv::Point _center,
                                                    float _resize_scale) {
  if (_resize_scale < 0)
    resize_scale = 1.0;
  ...
  std::cout << "[#INFO] ROI row: start " << roi_row_start << " ,end "
            << roi_row_end << std::endl;
  std::cout << "[#INFO] ROI col: start " << roi_col_start << " ,end "
            << roi_col_end << std::endl;
}

cv::Mat cv_utils::fisheye::PreProcess::do_preprocess(cv::Mat image_input) {
  if (is_resize_only) {
    cv::Mat image_input_resized;
    cv::resize(image_input, image_input_resized,
    ...
    cv::Mat image_input_roi =
        image_input(cv::Range(roi_row_start, roi_row_end),
