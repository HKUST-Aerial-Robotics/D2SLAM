...
#pragma omp parallel for  num_threads(5)
            for (unsigned int i = 0; i < undistMaps.size(); i++) {
                if (!disable[i]) {
                    cv::remap(gray, ret[i], undistMaps[i].first, undistMaps[i].second, REMAP_FUNC);
                }
            }
            return ret;
        }
...
#pragma omp parallel for num_threads(10)
    ...
    std::vector<std::pair<cv::Mat, cv::Mat>> generateCylinderMap(camodocal::CameraPtr p_cam,
                                          Eigen::Vector3d rotation,
                                          const unsigned &imgWidth,
        ...
        cam_top = camodocal::CylindricalCameraPtr( new camodocal::CylindricalCamera("cylindrical",
                  imgWidth, imgHeight,
                  f_center, f_center, imgWidth/2, imgHeight/2));
         
        maps.push_back(genOneUndistMap(0, p_cam, cam_top, t[0], imgWidth, imgHeight));
        return maps;
    }
...
std::pair<cv::Mat, cv::Mat> genOneUndistMap(
        ...
        for (unsigned int x = 0; x < imgWidth; x++)
            for (unsigned int y = 0; y < imgHeight; y++)
            {
                ...
                if (x == 0 && y == 0) {
                    ROS_INFO("LiftProjective (%d,%d) to (%f,%f,%f)", x, y, objPoint[0], objPoint[1], objPoint[2]);
                }
                Eigen::Vector2d imgPoint;
