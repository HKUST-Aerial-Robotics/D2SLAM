 ...
 public:
  SwarmLoopSpy(ros::NodeHandle &nh) {
    ...
    loop_net->loopconn_callback = [&](const LoopEdge_t &loop_conn) {
      ROS_INFO("Received loop from %d to %d", loop_conn.drone_id_a,
               loop_conn.drone_id_b);
    };

    timer = nh.createTimer(ros::Duration(0.03), &SwarmLoopSpy::timer_callback,
                           this);
  }

  void onLoopConnection(LoopEdge &loop_con, bool is_local = false) {
    ROS_INFO("Loop conn from %d to %d", loop_con.drone_id_a,
             loop_con.drone_id_b);
  }
  ...
  void timer_callback(const ros::TimerEvent &e) {
    for (auto &it : all_images) {
      ...
      auto nowPts = img_desc.images[1].landmarks2D();
      ...
      cv::putText(ret, frame_name, cv::Point2f(10, 10), cv::FONT_HERSHEY_PLAIN,
                  0.8, cv::Scalar(0, 255, 0));

      sprintf(frame_name, "Landmark num %ld", img_desc.landmarkNum());
      cv::putText(ret, frame_name, cv::Point2f(10, 20), cv::FONT_HERSHEY_PLAIN,
                  0.8, cv::Scalar(0, 255, 0));
...
int main(int argc, char **argv) {
  ...
  std::thread thread([&] {
    while (0 == loopnode.loop_net->lcmHandle()) {
    }
  });
  ros::spin();
