...
void D2Frontend::monoImageCallback(const sensor_msgs::ImageConstPtr &image) {
    ...
    for (int i = 0; i < 4; i++) {
        imgs.emplace_back(img(cv::Rect(i * img.cols / num_imgs, 0,
                                       img.cols / num_imgs, img.rows)));
                    ...
                    [&](VisualImageDescArray frame) {
                        ...
                        while (count < 1000) {
                            if (loop_detector->hasFrame(frame.matched_frame)) {
                                SPDLOG_INFO("Frame {} waited {} us for matched "
                                            "frame {}",
...
void D2Frontend::Init(ros::NodeHandle &nh) {
    ...
    loop_net->frame_desc_callback =
        [&](const VisualImageDescArray &frame_desc) {
            if (received_image) {
    ...
    image_transport::TransportHints hints(
        format, ros::TransportHints().tcpNoDelay(true));
    ...
    if (params->camera_configuration == CameraConfig::STEREO_PINHOLE ||
        ...
        SPDLOG_INFO("Input: raw images {} and depth {}",
                    params->image_topics[0], params->depth_topics[0]);
        image_sub_l =
            new ImageSubscriber(*it_, params->image_topics[0], 1000, hints);
        image_sub_r =
    ...
    if (params->enable_pub_remote_frame) {
        remote_image_desc_pub = nh.advertise<swarm_msgs::ImageArrayDescriptor>(
            "remote_frame_desc", 10);
    }
