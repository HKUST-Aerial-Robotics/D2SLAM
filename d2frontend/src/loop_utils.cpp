...
Eigen::MatrixXf load_csv_mat_eigen(std::string csv) {
    ...
    while (getline(infile, line)) {
        ...
        rows++;
        if (cols > 0) {
            assert(cols == temp_cols &&
                   "Matrix must have same cols on each rows!");
        } else {
            cols = temp_cols;
        }
    }
...
std::vector<cv::DMatch> matchKNN(const cv::Mat &desc_a, const cv::Mat &desc_b,
    ...
    for (auto &match : matches) {
        ...
        if (match[0].distance < knn_match_ratio * match[1].distance && match_inv_dict[match[0].trainIdx] == match[0].queryIdx) {
            if (search_local_dist > 0) {
                if (cv::norm(pts_a[match[0].queryIdx] -
                             pts_b[match[0].trainIdx]) > search_local_dist) {
                    continue;
                }
            }
...
std::vector<cv::Point2f> opticalflowTrack(const cv::Mat &cur_img,
    ...
    if (enable_cuda) {
        ...
        cv::Ptr<cv::cuda::SparsePyrLKOpticalFlow> d_pyrLK_sparse =
            cv::cuda::SparsePyrLKOpticalFlow::create(WIN_SIZE, PYR_LEVEL, 30,
                                                     true);
        d_pyrLK_sparse->calc(gpu_prev_img, gpu_cur_img, gpu_prev_pts,
                             gpu_cur_pts, gpu_status);
        ...
        cv::calcOpticalFlowPyrLK(
            prev_img, cur_img, prev_pts, cur_pts, status, err, WIN_SIZE,
            PYR_LEVEL,
...
void detectPoints(const cv::Mat &img, std::vector<cv::Point2f> &n_pts,
    ...
    if (lack_up_top_pts > require_pts / 4) {
        ...
        cv::Mat d_prevPts;
        if (use_fast) {
            n_pts_tmp = detectFastByRegion(img, mask, num_to_detect, fast_rows,
                                           fast_cols);
        } else {
            ...
            if (enable_cuda) {
                cv::Ptr<cv::cuda::CornersDetector> detector =
                    cv::cuda::createGoodFeaturesToTrackDetector(
                        img.type(), num_to_detect, 0.01,
                        params->feature_min_dist);
...
bool pnp_result_verify(bool pnp_success, int inliers, double rperr,
    ...
    success =
        (inliers >= _config.loop_inlier_feature_num) &&
...
int computeRelativePosePnP(const std::vector<Vector3d> lm_positions_a,
    ...
    for (int i = 0; i < lm_positions_a.size(); i++) {
        ...
        if (z > 1e-1) {
            pts3d.push_back(cv::Point3f(lm_positions_a[i].x(),
                                        lm_positions_a[i].y(),
                                        lm_positions_a[i].z()));
            pts2d.push_back(
    ...
    bool success =
        solvePnPRansac(pts3d, pts2d, K, D, rvec, t, false, iteratives,
    ...
    DP_b_to_a =
        Swarm::Pose::DeltaPose(pnp_predict_pose_b, ego_motion_a, is_4dof);
    if (verify_gravity) {
        auto RPerr = gravityCheck(pnp_predict_pose_b, ego_motion_b);
        success = pnp_result_verify(success, inliers.size(), RPerr, DP_b_to_a);
        printf("[SWARM_LOOP@%d] DPose %s PnPRansac %d inlines %d/%d, dyaw %f "
               "dpos %f g_err %f \n",
               params->self_id, DP_b_to_a.toStr().c_str(), success,
               inliers.size(), pts2d.size(), fabs(DP_b_to_a.yaw()) * 57.3,
               DP_b_to_a.pos().norm(), RPerr);
    }
    return success;
}
...
int computeRelativePosePnPnonCentral(
    ...
    DP_b_to_a =
        Swarm::Pose::DeltaPose(pnp_predict_pose_b, drone_pose_a, is_4dof);
