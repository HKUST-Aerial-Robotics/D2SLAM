...
bool ProjectionOneFrameTwoCamFactor::Evaluate(double const *const *parameters,
  ...
  // std::cout << "Source ERROR" << (pts_camera_j.normalized() -
  // pts_j_td.normalized()).transpose() << std::endl;
  ...
  if (jacobians) {
    ...
    if (jacobians[0]) {
      ...
      jaco_ex.rightCols<3>() =
          ric2_t_ric * -Utility::skewSymmetric(pts_camera_i);
    ...
    if (jacobians[2]) {
      ...
      jacobian_feature =
          reduce * ric2_t_ric * pts_i_td * -1.0 / (inv_dep_i * inv_dep_i);
    }
    if (jacobians[3]) {
#ifdef UNIT_SPHERE_ERROR
