...
void D2Estimator::init(ros::NodeHandle &nh, D2VINSNet *net) {
    ...
    if (params->estimation_mode == D2Common::DISTRIBUTED_CAMERA_CONSENUS) {
        solver =
            new D2VINSConsensusSolver(this, &state, sync_data_receiver,
...
bool D2Estimator::tryinitFirstPose(VisualImageDescArray &frame) {
    auto ret = imu_bufs[self_id].periodIMU(-1, frame.stamp +
                                                   state.getTd(frame.drone_id));
...
std::pair<bool, Swarm::Pose>
D2Estimator::initialFramePnP(const VisualImageDescArray &frame,
    ...
    if (frame.drone_id != self_id) {
        SPDLOG_INFO("D{} PnP succ {} frame {}@{} final {} inliers {} points {} "
                    "time: {:.2f}ms",
                    self_id, success, frame.frame_id, frame.drone_id,
                    pose_imu.toStr(), inliers.size(), lm_positions_a.size(),
                    tic.toc());
    }
            ...
            if (imu_[i].t > t_last) {
                if (add_first) {
                    if ((imu_[i].t - t_last) > params->max_imu_time_err) {
                        SPDLOG_INFO("Add remote imu buffer {}: dt {:.2f}ms",
...
VINSFrame *D2Estimator::addFrameRemote(const VisualImageDescArray &_frame) {
    ...
    if (state.size(r_drone_id) > 0) {
        ...
        if (params->estimation_mode == D2Common::SOLVE_ALL_MODE ||
            ...
            auto ret = imu_buf.periodIMU(last_frame.imu_buf_index,
                                         _frame.stamp + state.td);
            auto _imu = ret.first;
            if (fabs(_imu.size() / (_frame.stamp - last_frame.stamp) -
                     params->IMU_FREQ) > 15) {
                SPDLOG_WARN("Remote IMU error freq: {:.3f} start_t "
                            "{:.3f}/{:.3f} end_t {:.3f}/{:.3f}",
                            _imu.size() / (_frame.stamp - last_frame.stamp),
                            last_frame.stamp + state.td, _imu[0].t,
        ...
        if (!pnp_init.first) {
            ...
            if (_frame.reference_frame_id < state.getReferenceFrameId() &&
                ...
                SPDLOG_INFO("Merge map to reference frame {}@{} RP: {}",
                            _frame.reference_frame_id, _frame.drone_id,
                            P_w_ki.toStr());
            } else {
...
void D2Estimator::solveinDistributedMode() {
    ...
    if (params->enable_perf_output) {
        SPDLOG_INFO("D{} average time {:.1f}ms, average time of iter: "
                    "{:.1f}ms, average iteration {:.3f}, average cost {:.3f}",
                    self_id, sum_time * 1000 / solve_count,
                    sum_time * 1000 / sum_iteration,
...
void D2Estimator::solveNonDistrib() {
    if (params->enable_sfm_initialization) {
        if (state.numKeyframes() < params->min_solve_frames) {
            ...
            if (!isInitialized()) {
                SPDLOG_INFO("Initialization with {} keyframes",
                            state.numKeyframes());
                if (state.monoInitialization()) {
                    SPDLOG_INFO(
                        "Mono initialization is success, turn to solve");
                } else {
    ...
    if (params->enable_perf_output) {
        SPDLOG_INFO("average time {:.1f}ms, average time of iter: {:.1f}ms, "
                    "average iteration {:.3f}, average cost {:.3f}\n",
                    sum_time * 1000 / solve_count,
                    sum_time * 1000 / sum_iteration,
                    sum_iteration / solve_count, sum_cost / solve_count);
    }
    ...
    if (params->estimation_mode < D2Common::SERVER_MODE) {
        ...
        SPDLOG_INFO("C{} landmarks {} td {:1.f}ms opti_time {:.1f}ms",
                    solve_count, current_landmark_num, state.td * 1000,
        ...
        for (auto drone_id : state.availableDrones()) {
            ...
            if (state.size(drone_id) > 1) {
                for (size_t i = 0; i < state.size(drone_id) - 1; i++) {
                    ...
                    auto pre_integrations =
                        frame_b.pre_integrations; // Prev to current
                    if (pre_integrations == nullptr) {
                        SPDLOG_WARN(
                            "frame {}<->{}@{} pre_integrations is nullptr.",
                            frame_a.frame_id, frame_b.frame_id, drone_id);
                        continue;
                    }
                    assert(frame_b.prev_frame_id == frame_a.frame_id &&
                           "Wrong prev frame id on remote");
                    addIMUFactor(frame_a.frame_id, frame_b.frame_id,
                                 pre_integrations);
                }
            }
        }
...
void D2Estimator::setupLandmarkFactors() {
    ...
    for (auto it : keyframe_measurements) {
        ...
        if (it.second < params->min_measurements_per_keyframe) {
            ...
            if (frame->drone_id != self_id &&
                ...
                SPDLOG_WARN(
                    "D{} frame_id {} has only {} measurement, will be skip.",
    ...
    for (auto lm : lms) {
        ...
        if (firstObs.depth_mea && params->fuse_dep &&
            ...
            auto info = DepthResInfo::create(f_dep, loss_function,
                                             firstObs.frame_id, lm_id);
            marginalizer->addResidualInfo(info);
            solver->addResidual(info);
        ...
        for (auto i = 1; i < lm.track.size(); i++) {
            auto lm_per_frame = lm.track[i];
            if (ignore_frames.find(lm_per_frame.frame_id) !=
                ignore_frames.end()) {
                continue;
            }
            ...
            if (lm_per_frame.camera_id == base_camera_id) {
                ...
                if (firstObs.frame_id == lm_per_frame.frame_id) {
                    SPDLOG_WARN("landmarkid {} frame {}<->{}@{} is the same "
                                "camera id {}",
                ...
                info = LandmarkTwoFrameOneCamResInfo::create(
                    f_td, loss_function, firstObs.frame_id,
                    lm_per_frame.frame_id, lm_id, firstObs.camera_id,
                    enable_depth_mea);
            } else {
                if (lm_per_frame.frame_id == firstObs.frame_id) {
                    ...
                    info = LandmarkOneFrameTwoCamResInfo::create(
                        f_td, loss_function, firstObs.frame_id, lm_id,
...
const std::map<LandmarkIdType, LandmarkPerId> &
D2Estimator::getLandmarkDB() const {
...
D2Estimator::getMotionPredict(double stamp) const {
    if (!initFirstPoseFlag) {
        return std::make_pair(Swarm::Odometry(),
                              std::make_pair(IMUBuffer(), -1));
    }
    ...
    if (fabs(_imu.size() / (stamp - last_frame.stamp) - params->IMU_FREQ) >
        15) {
        SPDLOG_WARN("Local IMU error freq: {:.3f} start_t {:.3f}/{:.3f} end_t "
                    "{:.3f}/{:.3f}",
                    _imu.size() / (stamp - last_frame.stamp),
                    last_frame.stamp + state.td, _imu[0].t, stamp + state.td,
                    _imu[_imu.size() - 1].t);
    }
