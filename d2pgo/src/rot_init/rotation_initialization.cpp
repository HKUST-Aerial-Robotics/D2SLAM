...
namespace D2PGO {
RotInit::RotInit(PGOState * _state, RotInitConfig _config, ARockSolverConfig arock_config, 
        bool _enable_arock, std::function<void(const DPGOData &)> _broadcastDataCallback):
    enable_float32(_config.enable_float32), 
    enable_arock(_enable_arock) {
    if (enable_arock) {
        if (enable_float32) {
            rot_int = new RotationInitARockf(_state, _config, arock_config, _broadcastDataCallback);
            printf("[D2PGO::RotInit] Mode: ARock + float32\n");
        } else {
            rot_int = new RotationInitARockd(_state, _config, arock_config, _broadcastDataCallback);
...
void RotInit::setFixedFrameId(FrameIdType _fixed_frame_id) {
    if (enable_arock) {
        if (enable_float32) {
            static_cast<RotationInitARockf*>(rot_int)->setFixedFrameId(_fixed_frame_id);
        } else {
            static_cast<RotationInitARockd*>(rot_int)->setFixedFrameId(_fixed_frame_id);
        }
    } else {
        if (enable_float32) {
            static_cast<RotationInitializationf*>(rot_int)->setFixedFrameId(_fixed_frame_id);
        } else {
            static_cast<RotationInitializationd*>(rot_int)->setFixedFrameId(_fixed_frame_id);
        }
    }
}
